-- FastTrack v1 schema. Run in Supabase SQL editor as postgres.
-- Apply rls.sql immediately after this file.
...
create index if not exists documents_applicant_idx on public.documents (applicant_id);
...
stable
as $$
  select coalesce((auth.jwt() -> 'app_metadata' ->> 'role') = 'officer', false);
$$;
